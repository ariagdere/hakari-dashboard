...
// Pullback sekmesi -- naif ile AYNI naive_dist_ratio bucket'larini kullanir
// (bu, cluster geometrisine ait, giris mekanizmasindan bagimsiz bir metrik),
// ama sonuc sim_result_pullback / pullback_sim_r_multiple bazli.
...
export async function GET(req: NextRequest) {
  ...
  const { rows: rRows } = await pool.query(`
    ...
    ${base} naive_dist_ratio IS NOT NULL
      AND sim_result_pullback IN ('TP_HIT','SL_HIT')
      AND pullback_sim_r_multiple IS NOT NULL
  ...
  for (const r of rRows) {
    ...
    bucketRMap[r.bucket].push(parseFloat(r.pullback_sim_r_multiple))
  }
