  ...
  const { rows } = await pool.query(`
    SELECT
      CASE
        ...
        WHEN EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60 < 360  THEN '4-6sa'
        WHEN EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60 < 480  THEN '6-8sa'
      ...
      CASE
        ...
        WHEN EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60 < 360  THEN 6
        WHEN EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60 < 480  THEN 7
        WHEN EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60 < 1440 THEN 8
        ELSE 9
      END AS sort_order,
