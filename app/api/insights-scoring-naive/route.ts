...
// insights-scoring'in naif paraleli — sadece RSI kısmı (market_score_value /
  ...
  const [rsiRows, rsiLongRows, rsiShortRows, rsi30Rows, rsi30LongRows, rsi30ShortRows] = await Promise.all([
    pool.query(`
      SELECT ${RSI_CASE} AS rsi_zone, ${RSI_ORDER} AS sort_order,
        ...
        ROUND(COUNT(*) FILTER (WHERE sim_result_naive = 'TP_HIT') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE sim_result_naive IN ('TP_HIT','SL_HIT')), 0), 1) AS win_rate,
