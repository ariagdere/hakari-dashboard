...
export async function GET(req: NextRequest) {
  ...
  const [bucketRows, dirRows] = await Promise.all([
    pool.query(`
      SELECT
        CASE
          WHEN win_probability_v3 < 20 THEN '0-20%'
          WHEN win_probability_v3 < 30 THEN '20-30%'
          WHEN win_probability_v3 < 40 THEN '30-40%'
          WHEN win_probability_v3 < 50 THEN '40-50%'
          WHEN win_probability_v3 < 60 THEN '50-60%'
          WHEN win_probability_v3 < 70 THEN '60-70%'
          ELSE '70%+'
        END AS bucket,
        CASE
          WHEN win_probability_v3 < 20 THEN 1
          WHEN win_probability_v3 < 30 THEN 2
          WHEN win_probability_v3 < 40 THEN 3
          WHEN win_probability_v3 < 50 THEN 4
          WHEN win_probability_v3 < 60 THEN 5
          WHEN win_probability_v3 < 70 THEN 6
          ELSE 7
        END AS sort_order,
        ROUND(AVG(win_probability_v3), 1) AS avg_predicted,
      ...
      FROM btc_analysis
      ${base} win_probability_v3 IS NOT NULL
    ...
    pool.query(`
      SELECT direction,
        ROUND(AVG(win_probability_v3), 1) AS avg_probability,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')) AS total,
        ROUND(COUNT(*) FILTER (WHERE sim_result = 'TP_HIT') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')), 0), 1) AS actual_win_rate
      FROM btc_analysis
