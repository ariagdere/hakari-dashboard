...
function pairsQuery(direction: string, base: string, params: any[]) {
  return pool.query(`
    WITH pairs AS (
      ...
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_h1_tt_positions IS NOT NULL
      ...
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_m5_tt_positions IS NOT NULL
      ...
      SELECT 'mtf × h1_oi',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_h1_oi IS NOT NULL
      ...
      SELECT 'mtf × liquidity',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_liquidity IS NOT NULL
      ...
      SELECT 'mtf × market_power',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_market_power IS NOT NULL
      ...
      SELECT 'h1_ttp × m5_ttp',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_h1_tt_positions IS NOT NULL AND sent_m5_tt_positions IS NOT NULL
      ...
      SELECT 'liquidity × market_power',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_liquidity IS NOT NULL AND sent_market_power IS NOT NULL
      ...
      SELECT 'h1_oi × m5_oi',
        ...
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_h1_oi IS NOT NULL AND sent_m5_oi IS NOT NULL
      GROUP BY sent_h1_oi, sent_m5_oi
    )
    SELECT pair_name, combination, total, wins,
      ROUND(wins * 100.0 / NULLIF(total, 0), 1) AS win_rate
    FROM pairs WHERE total >= 5
    ORDER BY win_rate DESC, total DESC LIMIT 8
  `, params)
}

function triosQuery(direction: string, base: string, params: any[]) {
  return pool.query(`
    WITH trios AS (
      SELECT 'mtf × h1_ttp × m5_ttp' AS trio_name,
        'mtf=' || sent_synthesis_mtf || ' + h1_ttp=' || sent_h1_tt_positions || ' + m5_ttp=' || sent_m5_tt_positions AS combination,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')) AS total,
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT') AS wins
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_h1_tt_positions IS NOT NULL AND sent_m5_tt_positions IS NOT NULL
      GROUP BY sent_synthesis_mtf, sent_h1_tt_positions, sent_m5_tt_positions

      UNION ALL

      SELECT 'mtf × h1_oi × liquidity',
        'mtf=' || sent_synthesis_mtf || ' + h1_oi=' || sent_h1_oi || ' + liq=' || sent_liquidity,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')),
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_h1_oi IS NOT NULL AND sent_liquidity IS NOT NULL
      GROUP BY sent_synthesis_mtf, sent_h1_oi, sent_liquidity

      UNION ALL

      SELECT 'mtf × liquidity × market_power',
        'mtf=' || sent_synthesis_mtf || ' + liq=' || sent_liquidity || ' + mktpwr=' || sent_market_power,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')),
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_liquidity IS NOT NULL AND sent_market_power IS NOT NULL
      GROUP BY sent_synthesis_mtf, sent_liquidity, sent_market_power

      UNION ALL

      SELECT 'mtf × h1_ttp × market_power',
        'mtf=' || sent_synthesis_mtf || ' + h1_ttp=' || sent_h1_tt_positions || ' + mktpwr=' || sent_market_power,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')),
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_synthesis_mtf IS NOT NULL AND sent_h1_tt_positions IS NOT NULL AND sent_market_power IS NOT NULL
      GROUP BY sent_synthesis_mtf, sent_h1_tt_positions, sent_market_power

      UNION ALL

      SELECT 'h1_ttp × m5_ttp × liquidity',
        'h1_ttp=' || sent_h1_tt_positions || ' + m5_ttp=' || sent_m5_tt_positions || ' + liq=' || sent_liquidity,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')),
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT')
      FROM btc_analysis
      ${base} direction = '${direction}' AND sent_h1_tt_positions IS NOT NULL AND sent_m5_tt_positions IS NOT NULL AND sent_liquidity IS NOT NULL
      GROUP BY sent_h1_tt_positions, sent_m5_tt_positions, sent_liquidity
    )
    SELECT trio_name, combination, total, wins,
      ROUND(wins * 100.0 / NULLIF(total, 0), 1) AS win_rate
    FROM trios WHERE total >= 5
    ORDER BY win_rate DESC, total DESC LIMIT 6
  `, params)
}
...
export async function GET(req: NextRequest) {
  ...
  const [dirSentRows, scoreSentRows, longPairs, shortPairs, longTrios, shortTrios] = await Promise.all([
    pool.query(`
      SELECT direction, sent_synthesis_mtf AS mtf_strength,
        COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')) AS total,
        COUNT(*) FILTER (WHERE sim_result = 'TP_HIT') AS wins,
        ROUND(COUNT(*) FILTER (WHERE sim_result = 'TP_HIT') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')), 0), 1) AS win_rate
      FROM btc_analysis
      ${base} direction IN ('LONG','SHORT') AND sent_synthesis_mtf IS NOT NULL AND sim_result IN ('TP_HIT','SL_HIT')
      GROUP BY direction, sent_synthesis_mtf ORDER BY direction, win_rate DESC
    `, params),

    pool.query(`
      SELECT
        CASE WHEN market_score_value <= 4 THEN 'Düşük (1-4)'
             WHEN market_score_value BETWEEN 5 AND 7 THEN 'Orta (5-7)'
             ELSE 'Yüksek (8-10)' END AS score_bucket,
        ...
        ROUND(COUNT(*) FILTER (WHERE sim_result = 'TP_HIT') * 100.0 /
          NULLIF(COUNT(*) FILTER (WHERE sim_result IN ('TP_HIT','SL_HIT')), 0), 1) AS win_rate
      FROM btc_analysis
      ${base} market_score_value IS NOT NULL AND sent_synthesis_mtf IS NOT NULL AND sim_result IN ('TP_HIT','SL_HIT')
      GROUP BY score_bucket, sent_synthesis_mtf ORDER BY score_bucket, win_rate DESC
    `, params),

    pairsQuery('LONG',  base, params),
    pairsQuery('SHORT', base, params),
    triosQuery('LONG',  base, params),
    triosQuery('SHORT', base, params),
  ])
