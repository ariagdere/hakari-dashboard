...
export async function GET(req: NextRequest) {
  ...
  const q = await pool.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE direction = 'LONG' AND sim_result IN ('TP_HIT','SL_HIT','EXPIRED','NO_ENTRY')) AS long_sim,
      ROUND(
      ...
      COUNT(*) FILTER (WHERE direction = 'SHORT' AND sim_result IN ('TP_HIT','SL_HIT','EXPIRED','NO_ENTRY')) AS short_sim,
      ROUND(
