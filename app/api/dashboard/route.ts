  ...
  const [statsResult, rSeriesResult, activeTradesResult] = await Promise.all([
    pool.query(`
      SELECT
        ...
        ROUND(AVG(rsi_4h) FILTER (WHERE sim_result = 'TP_HIT' AND direction = 'SHORT'), 1) AS avg_rsi_tp_short,
        ROUND(AVG(rsi_4h) FILTER (WHERE sim_result = 'TP_HIT' AND direction = 'LONG'), 1) AS avg_rsi_tp_long,
        ...
        ROUND(AVG(EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60) FILTER (WHERE sim_result = 'TP_HIT'), 0) AS avg_mins_to_entry_tp,
        ...
        ROUND(AVG(rsi_4h) FILTER (WHERE sim_result = 'SL_HIT' AND direction = 'SHORT'), 1) AS avg_rsi_sl_short,
        ROUND(AVG(rsi_4h) FILTER (WHERE sim_result = 'SL_HIT' AND direction = 'LONG'), 1) AS avg_rsi_sl_long,
        ...
        ROUND(AVG(EXTRACT(EPOCH FROM (sim_entry_triggered_at - analyzed_at)) / 60) FILTER (WHERE sim_result = 'SL_HIT'), 0) AS avg_mins_to_entry_sl,
