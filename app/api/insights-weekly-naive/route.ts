...
// insights-weekly'nin naif paraleli — sim_result_naive / naive_sim_r_multiple bazlı.
...
export async function GET(req: NextRequest) {
  ...
  const byDayQ = `
    SELECT
      ...
      ROUND(COUNT(*) FILTER (WHERE sim_result_naive = 'TP_HIT') * 100.0 / NULLIF(COUNT(*), 0), 1) AS win_rate,
      ROUND(SUM(naive_sim_r_multiple)::numeric, 2) AS total_r
    FROM btc_analysis
